...
public class BTFromInorderDFS {
	...
	public static void main(String[] args) {
		...
		int[] DFS = { 10, 5, 2, 6, 14, 12, 15 };
		Node root = constructTree(inOrder, DFS, 0, inOrder.length - 1);
		printInorder(root);
	}

	public static Node constructTree(int[] inOrder, int[] DFS, int start, int end) {
		if (start > end) {
			return null;
		}
		int rootVal = DFS[pindex];
		pindex++;
		...
		root.left = constructTree(inOrder, DFS, start, inorderIndex - 1);
		root.right = constructTree(inOrder, DFS, inorderIndex + 1, end);
		return root;
	}
